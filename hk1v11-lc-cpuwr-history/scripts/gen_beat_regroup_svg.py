...
def beat(x, y, beat_label, cells, fold, frame_color):
    ...
    s = f'<rect x="{x}" y="{y}" width="{w}" height="{CELL_H+2*BEAT_PAD}" rx="9" fill="none" stroke="{frame_color}" stroke-width="2"/>'
...
parts = []
parts.append('<svg xmlns="http://www.w3.org/2000/svg" width="1240" height="848" viewBox="0 0 1240 848" font-family="Calibri,Segoe UI,Arial,sans-serif">')
parts.append('<rect width="1240" height="848" fill="#ffffff"/>')
...
parts.append('<line x1="620" y1="110" x2="620" y2="740" stroke="#e2e8f0" stroke-width="2"/>')
...
OLD2 = BEAT_PAD*2 + 2*CELL_W + CELL_GAP      # 2-cell beat stride base
OLD3 = BEAT_PAD*2 + 3*CELL_W + 2*CELL_GAP    # 3-cell beat stride base
...
# ---------------- Zone B: 0x1B~0x20 ----------------
zone(322, '区域 B：0x1B ~ 0x20（跨 Segment1 边界，0721 后 0x1F/0x20 同拍）')
y = 356
old_b = [
    ("5'h0F", [("1C",G),("1D",I)], "D"),
    ("5'h10", [("1E",I),("1F",I)], "C^D"),
...
zone(486, '区域 C：0x27（checksum frame end；每帧最后一笔参与字节）')
y = 520
old_c = [
...
# ---------------- Zone D: LCONFIG ----------------
zone(650, '区域 D：LCONFIG 0x40 ~ 0x44（6B：3 拍 → 2 拍，无 pad）')
y = 684
old_l = [
    ("3'h1", [("40",I),("41",I)], "A,B"),
    ("3'h2", [("42",I),("43",I)], "A,B"),
...
# ---------------- Summary ----------------
parts.append('<rect x="48" y="768" width="560" height="66" rx="10" fill="#ecfdf5" stroke="#16a34a" stroke-width="1.5"/>')
parts.append('<text x="64" y="790" font-size="13" font-weight="700" fill="#166534">XOR 总量不变（结合律）</text>')
parts.append('<text x="64" y="808" font-size="12" fill="#166534">0x00^01^02^03^04^05^06^08^09^0A^1D^1E^1F，frame_end=0x27</text>')
parts.append('<text x="64" y="826" font-size="12" fill="#166534">分组变化不改值 → checker / DV 期望与回归行为不变</text>')
parts.append('<rect x="640" y="768" width="560" height="66" rx="10" fill="#fff7ed" stroke="#fb923c" stroke-width="1.5"/>')
parts.append('<text x="656" y="790" font-size="13" font-weight="700" fill="#c2410c">过渡依赖</text>')
parts.append('<text x="656" y="808" font-size="12" fill="#c2410c">CHPI_CONFIG_L0 未升 24bit 前，LINK_LOGIC 将 [23:16] tie-off 防 X 传播</text>')
parts.append('<text x="656" y="826" font-size="12" fill="#c2410c">全芯片 FCONFIG 流仿真待上游升版后联调（END：5\'d20 / 3\'d3）</text>')
