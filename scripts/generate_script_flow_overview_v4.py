...
# -*- coding: utf-8 -*-
"""
Generate script-flow-overview.svg with orthogonal (straight + right-angle) routing.
If a straight line would cross other nodes, route it as two/three/multi-segment polylines.
"""
import os

OUT = os.path.join(os.path.dirname(__file__), '..', 'synthesis-integration-flow-guide', 'script-flow-overview-v4.svg')
...
def build_svg():
    ...
    for i, (frm, to, color, label, d, label_xy) in enumerate(EDGES):
        ...
        parts.append(f'    <path id="edge-{i}" d="{d}" fill="none" stroke="{COLOR[color]}" stroke-width="1.8" stroke-dasharray="{dash}" marker-end="url(#arrow-{color})" opacity="0.95"/>')
    parts.append('  </g>')
    ...
    for i, (frm, to, color, label, d, label_xy) in enumerate(EDGES):
        ...
        x, y = label_xy
        tw = len(label) * 10 + 10
        parts.append(f'    <rect x="{x - tw/2:.1f}" y="{y - 9:.1f}" width="{tw}" height="16" rx="8" fill="{LIGHT[color]}" stroke="{COLOR[color]}" stroke-width="0.5" opacity="0.95"/>')
