...
# -*- coding: utf-8 -*-
"""
Generate script-flow-overview.svg v6:
- taller canvas to eliminate label/node overlaps
- white nodes with layer-colored borders
- orthogonal routing with labels placed in clear areas
"""
import os

OUT = os.path.join(os.path.dirname(__file__), '..', 'synthesis-integration-flow-guide', 'script-flow-overview-v6.svg')
...
LAYERS = [
    ...
    ("L7 postPT",          672, 128, "violet"),
    ("L8 批量与汇总",      816, 84,  "slate"),
]
...
NODES = [
    ...
    ("write_sdf", 6, 720, 760, 170, 30, "write_sdf", "", True),
    ("parallel_run", 7, 140, 830, 260, 46, "pararrel_run_pt.py / run_all", "10 corner 并发 / 串行旧版", False),
    ("run_summary", 7, 480, 830, 180, 46, "run_summary", "", False),
    ("get_pl", 7, 740, 830, 150, 46, "get_*.pl", "抓数", False),
    ("merge_pl", 7, 940, 830, 150, 46, "merge_*.pl", "合并", False),
    ("report_csv", 7, 1130, 830, 130, 46, "report_*.csv", "", False),
]

# (from, to, color, label, path_d, label_xy)
EDGES = [
    ("design.set", "compile_dc", "purple", "source",
     "M 340,104 L 700,104 L 700,264 L 640,264", (520,104)),
    ("run_ckf", "run_dc", "gray", "",
    ...
    ("dc_rep_dir", "pt_setup_pre", "green", ".gv + ip_cons",
     "M 910,312 L 910,560 L 660,560 L 660,616", (785,560)),
    ...
    ("sdc_pr", "apr", "green", "供 APR",
     "M 1070,594 L 1090,544", (1100,570)),
    ("apr", "pt_setup_post", "green", "网表+spef+postcts SDC",
     "M 1090,544 L 1090,560 L 960,560 L 960,688", (1025,550)),
    ("run_pt_post", "corner_set", "blue", "corner 配置",
     "M 360,710 L 440,710", (400,702)),
    ...
    ("pt_setup_post", "write_sdf", "red", "DUMP_SDF=1",
     "M 840,732 L 805,760", (822,746)),
    ("run_pt_post", "parallel_run", "purple", "xterm + bsub 并发",
     "M 250,732 L 250,830 L 270,830", (260,781)),
    ("rpt_date", "run_summary", "blue", "latest",
     "M 1040,710 L 1040,800 L 660,800 L 660,830", (850,800)),
    ("run_summary", "get_pl", "blue", "",
     "M 660,853 L 740,853", None),
    ("get_pl", "merge_pl", "blue", "",
     "M 890,853 L 940,853", None),
    ("merge_pl", "report_csv", "green", "csv",
     "M 1090,853 L 1130,853", (1108,845)),
]
...
def build_svg():
    ...
    for label, y, h, key in LAYERS:
        pal = PAL[key]
        parts.append(f'  <rect x="20" y="{y}" width="1240" height="{h}" rx="10" ry="10" fill="{pal["light"]}" stroke="{pal["stroke"]}" stroke-width="1" stroke-opacity="0.35" stroke-dasharray="6,4"/>')
        bw = len(label) * 12 + 24
