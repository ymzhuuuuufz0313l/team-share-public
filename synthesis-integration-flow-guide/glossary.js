...
(function () {
  ...
  const glossary = {
    'design.set': '综合环境的总控配置文件，记录顶层名、工具选择、库文件、PVT corner、宏定义等关键参数，是 flow 各阶段读取的权威输入；综合与 postPT 各有一份，注意区分。',
    ...
    'set_false_path': '告知时序工具不对指定路径做建立/保持检查，常用于异步跨时钟域路径。修改位置：SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl（dco），改后需重跑 prePT 展开。',
    'set_multicycle_path': '允许指定路径在多个时钟周期内完成数据传输的时序约束命令。修改位置：SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl（dco），改后需重跑 prePT 展开。',
    ...
    'PTPX': '基于 PrimeTime 的功耗分析工具，通常使用 VCD 或 SAIF 作为开关活动输入。环境与配置目录：power/0.px_template/px_85c_random/（px_cfg.tcl 指定 ip0_vcd 等输入）。',
    'VCD': 'Value Change Dump，记录仿真过程中信号值变化的通用波形文件格式。',
    'SDF': 'Standard Delay Format，包含门级网表延迟反标信息的时序文件，用于后端 signoff 与后仿反标；产物目录 SYNTHESIS/top/sta/post/sdf/，仿真侧在 waves_dumper.sv 中以 $sdf_annotate 反标。',
    'SDC': 'Synopsys Design Constraints，时序约束文件，描述时钟、I/O 延迟、false path / multicycle 等约束；源头为 dco（SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl），prePT 展开产物放 sdc_pr/ 供 APR 使用，postPT 读 APR 返回的 postcts SDC（DataBase Release 目录）。',
    ...
    'false path': '不需要做时序检查的路径，用 set_false_path 声明；判定错误会把真实路径漏报，必须逐条人工确认。修改位置：SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl（dco 约束源文件），改后需重跑 prePT 展开，禁止直接改 SDC 产物。',
    'multicycle': '允许路径跨多个时钟周期完成数据传输的约束，用 set_multicycle_path 声明，常用于慢速控制路径。修改位置：SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl（dco），改后需重跑 prePT 展开。',
    'corner': 'PVT 仿真角的简称；当前工艺共 10 个 corner，setup / hold / transition 需全量校验，不做取舍。每 corner 配置文件：SYNTHESIS/top/sta/post/pt/<corner>/corner_set.tcl。',
    ...
    'dco': '集中书写 false path / multicycle / create_clock 等约束的源文件，真实路径 SYNTHESIS/common/ip_cons/HV2M23_digital_top.tcl（即 $IP_CONS_DIR/$TOP_NAME.tcl）；经 prePT 展开生成正式 SDC，禁止直接修改 SDC 产物。',
    'notimingcheck': '后仿时序屏蔽名单，真实路径 HV2M23/DV_TCON_C/script/notimingcheck_DIG.cfg；名单依据 prePT 报告逐条人工确认，改后只需重跑后仿，不需要重跑 DC / prePT。',
    'input_delay': '输入延迟约束，约束文件 ip_cons/input_delay.tcl；I2C 串行输入 max/min 成对书写，RX lane 数据整组通配并挂 OD/2 分频时钟。',
    'DUMP_SDF': 'corner_set.tcl 中的 SDF 生成开关；=1 时该趟 run 只执行 write_sdf 不产任何 timing report，取回 SDF 后必须改回 0，否则 run_summary 抓到空结果。',
    'waves_dumper.sv': '后仿 case 的波形与 SDF 反标配置，真实路径 DV_TCON_C/top/tests_netlist_post/<case>/waves_dumper.sv；其中三处 $sdf_annotate 路径须随 SDF 存放位置同步修改。',
  ...
  const cmdGlossary = {
    ...
    'update_latest': 'DataBase 交付目录（/HV2M23/DataBase/Digital/Release/HV2M23_digital_top_to_APR）下刷新 latest 指向的脚本；跑完后 latest/ 下应出现完整交付清单：.gv 网表、.sdc / _postcts.sdc、dont_touch.tcl、add_on*.tcl。',
    'newgrp': '切换用户组以提权的命令。用法：newgrp HV2M23_adm——DataBase 目录需先联系管理员（顾老师）开通权限，再用它切到项目管理员组后才能写入。'
  };
